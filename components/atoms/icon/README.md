...
A Bootstrap Icon renderer with size and color utilities, delegating to the `vartheme_bs5_horizonaid:bootstrap-icon` component.
...
```twig
{% include 'vartheme_bs5_horizonaid:icon' with {
  icon: 'people',
  icon_size: 'fs-4',
  color: 'text-body'
} only %}
...
- The icon is rendered through `vartheme_bs5_horizonaid:bootstrap-icon`, with `icon`, `icon_size`, and `color` passed through.
