...
- optionally render the heading text as a link via the `vartheme_bs5_horizonaid:link` component
...
- `url`: optional URL; when provided the heading text is rendered as a link using the `vartheme_bs5_horizonaid:link` component
...
- When `url` is provided, the heading content is delegated to the `vartheme_bs5_horizonaid:link` component; `target` is only passed through when it is not `_self`.
