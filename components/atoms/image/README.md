...
```twig
{% include 'vartheme_bs5_horizonaid:image' with {
  media: {
...
- Raster images are rendered responsively via `vartheme_bs5_horizonaid:dynamic-responsive-image` (drimage); SVGs bypass image-style derivatives and render as a native `<img>` at their intrinsic size to avoid stretching and 404s.
