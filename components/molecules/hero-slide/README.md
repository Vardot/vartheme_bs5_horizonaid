...
Use this component inside `vartheme_bs5_horizonaid:hero-slider-container` when you need a slide that can:

- render a title, rich-text content, and an optional CTA button (via `vartheme_bs5_horizonaid:button`)
...
```twig
{{ include('vartheme_bs5_horizonaid:hero-slide', {
  active: true,
...
- This component is meant to live inside `vartheme_bs5_horizonaid:hero-slider-container`; it reads the container context variables `hero_slider_media_position` (default media position) and `hero_slider_container_type` (content width for overlay slides).
...
- Images render through `vartheme_bs5_horizonaid:image`; a black fallback box is shown when an overlay/split slide has no media.
