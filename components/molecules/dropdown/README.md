...
```twig
{% include 'vartheme_bs5_horizonaid:dropdown-menu' with {
  items: menu.items,
  menu_level: 0
} only %}
```

Recursive submenus are produced automatically for items that are expanded and have a `below` array; the template re-includes `vartheme_bs5_horizonaid:dropdown-menu` for each nested level.
...
- The component depends on the `vartheme_bs5_horizonaid/bs-collapse-script` and `vartheme_bs5_horizonaid/bs-dropdown-script` libraries (declared via `libraryOverrides`).
