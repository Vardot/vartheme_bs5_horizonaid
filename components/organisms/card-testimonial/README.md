...
```twig
{{ include('vartheme_bs5_horizonaid:card-testimonial', {
  style: 'bg-body',
  shadow: 'shadow',
  align: 'text-center',
...
- The avatar is rendered via the `vartheme_bs5_horizonaid:image` component; it falls back to a 60×60 box and uses `cite_name` as the alt text when no `alt` is provided.
