...
- display a formatted published date via the `vartheme_bs5_horizonaid:date` component
- render the title through the `vartheme_bs5_horizonaid:heading` component
- show the author as plain text or as a linked name via `vartheme_bs5_horizonaid:link`
...
- `date`: ISO 8601 date string (`YYYY-MM-DD`); rendered via `vartheme_bs5_horizonaid:date`
...
```twig
{% include 'vartheme_bs5_horizonaid:hero-blog' with {
...
```twig
{% embed 'vartheme_bs5_horizonaid:hero-blog' with {
