/**
 * @file
 * Country Slider enhancements (Canvas/SDC-safe).
 *
...
((Drupal, once) => {
  function flattenTrackItems(track) {
    Array.prototype.slice
      .call(track.querySelectorAll('.card-country'))
      .forEach((item) => {
  ...
  function stepWidth(track) {
    const first = track.querySelector('.card-country');
    if (!first) {
  ...
  function initSlider(root) {
    const track = root.querySelector('[data-country-slider-track]');
    const prevButton = root.querySelector('[data-country-slider-prev]');
    const nextButton = root.querySelector('[data-country-slider-next]');
  ...
  Drupal.behaviors.countrySlider = {
    attach(context) {
      once('country-slider', '.country-slider', context).forEach(initSlider);
    },
  };
})(Drupal, once);
