...
- render images through the `vartheme_bs5_horizonaid:image` component
...
```twig
{{ include('vartheme_bs5_horizonaid:card', {
...
- A button is shown only when `url`, `button_label`, and a valid `button_variant` are all provided; the button is rendered through the `vartheme_bs5_horizonaid:button` component at `btn-sm`.
