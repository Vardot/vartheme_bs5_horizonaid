...
- render a heading (via the shared `vartheme_bs5_horizonaid:heading` component) with a chosen level and visual size
...
```twig
{% include 'vartheme_bs5_horizonaid:cta' with {
...
```twig
{% embed 'vartheme_bs5_horizonaid:cta' with {
...
- The background image is rendered through the shared `vartheme_bs5_horizonaid:image` component with `object-fit-cover` and `w-100`.
