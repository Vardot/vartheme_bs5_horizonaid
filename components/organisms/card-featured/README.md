...
```twig
{% embed 'vartheme_bs5_horizonaid:card-featured' with {
  card_border: true,
  padded: true,
...
{% embed 'vartheme_bs5_horizonaid:card-featured' with {
  ...
  {% block overlay %}
    {{ include('vartheme_bs5_horizonaid:badge', {
      label: 'Feature',
...
- The stretched link is rendered through the `vartheme_bs5_horizonaid:link` component with `stretched-link` and an `aria-label` derived from the content; it only activates when `stretched_link` is enabled and `link_url` is set. Avoid interactive elements in the content slot when using it.
- When the `content` slot is empty, a "Drop content here" placeholder is rendered for editing.
- Images are rendered through the `vartheme_bs5_horizonaid:image` component.
